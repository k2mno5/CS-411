...
urlpatterns = [
    ...
    url(r'^post/vote/(?P<postID>[0-9]+)/(?P<postType>[0-1]{1})/(?P<userID>[0-9]+)/(?P<voteStatus>[0-2]{1})/(?P<token>[0-9]+)/$',views.updateVoteStatus),
    url(r'^userUpdateRandom/$', views.getUserUpdate_random),
    url(r'^displayQuestionAnswers/(?P<qaID>[0-9]+)/(?P<is_answ>[0-1]{1})/$', 
        views.displayQuestionAnswers),
    url(r'^post/postAnswer/$',views.postAnswer),
    url(r'^post/postQuestion/$',views.postQuestion),
    url(r'^post/deletePost/(?P<ID>[0-9]+)/(?P<is_answ>[0-9]+)/(?P<userID>[0-9]+)/(?P<token>[0-9]+)/$',views.deletePost),
